import { exercises as english } from '../en/exercises';
import type { MessageShape } from '@/i18n/message-types';

export const exercises = {
  title: 'Каталог упражнений',
  savedCount:
    '{count, plural, =0 {Нет сохранённых упражнений.} one {Сохранено # упражнение.} few {Сохранено # упражнения.} many {Сохранено # упражнений.} other {Сохранено # упражнения.}}',
  search: 'Поиск упражнений по названию',
  gymFilter: 'Зал',
  allGyms: 'Все залы',
  muscleFilter: 'Группа мышц',
  allMuscles: 'Все мышцы',
  equipmentFilter: 'Тип оборудования',
  allEquipment: 'Всё оборудование',
  resetFilters: 'Сбросить фильтры',
  emptyTitle: 'Нет упражнений',
  emptyDescription: 'Каталог пуст. Добавьте первое упражнение, чтобы использовать его в программе.',
  noMatchTitle: 'Ничего не найдено',
  noMatchDescription: 'Упражнений по запросу «{query}» нет. Измените запрос.',
  noFilteredDescription: 'Нет упражнений для текущего поиска и фильтров. Измените или сбросьте их.',
  noGymExercisesTitle: 'Нет доступных упражнений',
  noGymExercisesDescription: 'В зале «{gym}» нет упражнений, отмеченных как доступные.',
  trainedDays: 'Тренировочных дней: {count}',
  restSeconds: 'отдых {seconds} с',
  editTitle: 'Изменить упражнение',
  addTitle: 'Добавить упражнение',
  formDescription: 'Укажите название, группу мышц и категорию.',
  muscleGroup: 'Группа мышц',
  category: 'Категория',
  equipmentType: 'Тип оборудования',
  physicalEquipment: 'Доступно на физическом оборудовании',
  physicalEquipmentDescription:
    'Выберите сохранённые тренажёры или станции, на которых можно выполнять это упражнение. Доступность определяется отдельно для каждого зала.',
  activeGym: 'Активный зал',
  makePreferred: 'Использовать «{equipment}» по умолчанию в этом зале',
  makePreferredAndChangeType:
    'Использовать «{equipment}» по умолчанию и изменить тип упражнения на «{type}»',
  differentEquipmentType: 'Другой тип оборудования',
  managedByBarbellProfile: 'Управляется профилем «Штанга»',
  equipmentConfirmationHint: 'Изменения применятся только после нажатия «Сохранить».',
  preferred: 'По умолчанию',
  defaultRest: 'Отдых по умолчанию (секунды)',
  bodyweight: 'Упражнение с собственным весом',
  bodyweightDescription:
    'Для подтягиваний, отжиманий и брусьев объём учитывает вес тела. Вводите только дополнительный вес; для противовеса укажите отрицательное значение.',
  notes: 'Заметки / инструкции (необязательно)',
  created: 'Упражнение создано.',
  updated: 'Упражнение обновлено.',
  deleted: 'Упражнение удалено.',
  saveError: 'Не удалось сохранить упражнение.',
  equipmentSaveError: 'Упражнение сохранено, но не удалось обновить связи с оборудованием.',
  deleteError: 'Не удалось удалить упражнение.',
  deleteTitle: 'Удалить упражнение?',
  deleteDescription:
    '«{name}» будет удалено из каталога. Удаление невозможно, пока упражнение используется в программе или истории тренировок.',
  deleting: 'Удаление...',
  media: {
    button: 'Техника',
    open: 'Посмотреть технику: {name}',
    description:
      'Начальный и конечный кадры переключаются автоматически. Поставьте на паузу, чтобы рассмотреть положение.',
    start: 'Старт',
    finish: 'Финиш',
    startAlt: 'Начальное положение: {name}',
    finishAlt: 'Конечное положение: {name}',
    showStart: 'Показать начальное положение',
    showFinish: 'Показать конечное положение',
    play: 'Включить анимацию',
    pause: 'Остановить анимацию',
    similarVariant: 'Похожий вариант',
    equipment: 'Оборудование',
    equipmentDescription:
      'Необходимое оборудование: {equipment}. На кадрах также видно, как выглядит и располагается снаряд или тренажёр.',
    disclaimer:
      'Используйте кадры как наглядный ориентир, а не замену работе с тренером. Прекратите движение при резкой боли.',
    source: 'Источник: {source} · {license}',
    missing: 'Для этого пользовательского упражнения пока нет проверенного локального медиа.',
    searchCommons: 'Найти на Wikimedia Commons',
  },
  detail: {
    back: 'Назад к упражнениям',
    backToSession: 'Назад к тренировке',
    information: 'Информация об упражнении',
    muscle: 'Основная мышечная группа',
    equipment: 'Оборудование',
    defaultRest: 'Обычный отдых',
    seconds: 'с',
    history: 'История тренировок',
    openChart: 'График прогресса',
    noHistory: 'Для этого упражнения пока нет выполненных подходов.',
    openSession: 'Открыть тренировку',
    edit: 'Изменить упражнение',
    editEquipment: 'Изменить оборудование',
    editEquipmentAria: 'Изменить оборудование. Тип: {type}. Выбрано: {equipment}.',
    editConcreteEquipmentAria: 'Изменить оборудование упражнения, начиная с «{equipment}»',
    broadEquipmentType: 'Общий тип оборудования',
    selectedEquipment: 'Выбранное конкретное оборудование',
    noPreferredEquipment: 'Выберите оборудование по умолчанию',
    noActiveGymShort: 'Нет активного зала',
    activeGymEquipment: 'Оборудование активного зала',
    noActiveGym: 'Выберите активный зал, чтобы назначить конкретное оборудование.',
    noEquipment: 'В зале «{gym}» с этим упражнением не связано оборудование.',
    openGymSettings: 'Открыть настройки зала',
    compatibleEquipment: 'Совместимое оборудование',
    baseLoadShort: 'пустой вес {weight} кг',
    weightsShort: '{weights} кг',
    baseLoad: 'Пустой вес: {weight} кг',
    loadingSides:
      '{count, plural, one {# нагружаемая сторона} few {# нагружаемые стороны} many {# нагружаемых сторон} other {# нагружаемой стороны}}',
    platePool: 'Группа блинов: {name}',
    availableWeights: 'Доступные веса: {weights} кг',
    displayedLoads: 'Значения на шкале: {weights} кг',
    selectorizedMultiplier: 'Значение на шкале × {multiplier}',
    loadTypes: {
      NONE: 'Без настроенной модели нагрузки',
      FIXED: 'Фиксированные веса',
      SELECTORIZED: 'Весовой стек / блочный тренажёр',
      PLATE_LOADED: 'Нагрузка блинами',
    },
    barbellFamilies: {
      LARGE: 'Большой / толстый диаметр',
      SMALL: 'Малый / тонкий диаметр',
    },
  },
  muscleGroups: {
    chest: 'Грудь',
    backWidth: 'Спина (ширина)',
    backThickness: 'Спина (толщина)',
    shouldersFront: 'Передние дельты',
    shouldersLateral: 'Средние дельты',
    shouldersRear: 'Задние дельты',
    biceps: 'Бицепс',
    triceps: 'Трицепс',
    forearms: 'Предплечья',
    quads: 'Квадрицепсы',
    hamstrings: 'Задняя поверхность бедра',
    glutes: 'Ягодицы',
    calves: 'Икры',
    abs: 'Пресс',
    lowerBack: 'Поясница',
    other: 'Другое',
  },
  categories: {
    compound: 'Базовое',
    isolation: 'Изолирующее',
    cardio: 'Кардио',
  },
  equipmentTypes: {
    dumbbell: 'Гантели',
    barbell: 'Штанга',
    machine: 'Тренажёр',
    cable: 'Блочный тренажёр',
    bodyweight: 'Собственный вес',
    cardio: 'Кардиооборудование',
    other: 'Другое / без ограничений',
  },
} satisfies MessageShape<typeof english>;
