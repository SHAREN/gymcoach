...
export function SetValuePicker({
  ...
  const t = useTranslations('session.valuePicker');
  ...
  const plateLoad = useMemo(() => {
    ...
    const plates = loadConstraints.plateWeights?.length
      ...
      : fallback.plates;
    return chooseBestPlateLoad(numericValue, bars, plates, fallback.barWeight);
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="left-0 top-0 flex h-[100dvh] max-h-none w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden rounded-none border-0 p-0 shadow-none sm:left-1/2 sm:top-1/2 sm:h-[90dvh] sm:max-h-[52rem] sm:max-w-md sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-md sm:border [&>button]:hidden">
        <header className="grid shrink-0 grid-cols-[5rem_1fr_5rem] items-center px-3 pb-2 pt-[calc(env(safe-area-inset-top)+0.75rem)]">
          <Button type="button" variant="ghost" onClick={onClose} className="justify-start px-1 text-primary">
            {t('cancel')}
          </Button>
          <DialogTitle className="text-center text-base">
            {kind === 'weight' ? t('chooseWeight', { unit: unit.toLowerCase() }) : t('chooseReps')}
          </DialogTitle>
        ...
        <div className="shrink-0 border-t border-border bg-background px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 shadow-[0_-8px_24px_rgba(0,0,0,0.18)]">
          <div className="grid min-h-[5rem] grid-cols-[minmax(0,1fr)_minmax(7rem,1.1fr)_4.25rem] items-center gap-2">
            {kind === 'weight' && plateLoad ? <BarbellSideDiagram load={plateLoad} unit={unit} /> : <div aria-hidden />}
            <div
              role="textbox"
              aria-readonly="true"
          ...
          <div data-testid="set-value-keypad" className="mt-3 grid grid-cols-3 gap-2" aria-label={t('keypad')}>
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((digit) => (
              <KeypadButton key={digit} onClick={() => appendKey(String(digit))}>{digit}</KeypadButton>
            ))}
            <KeypadButton onClick={() => appendKey('decimal')} disabled={kind === 'reps'}>{decimalLabel}</KeypadButton>
...
function KeypadButton({ children, disabled = false, label, onClick }: {
  children: ReactNode;
  disabled?: boolean;
  label?: string;
  onClick: () => void;
  ...
  return (
    <Button type="button" variant="secondary" disabled={disabled} onClick={onClick} aria-label={label} className="h-16 text-3xl font-semibold tabular-nums">
      {children}
    </Button>
  );
}

function chooseBestPlateLoad(target: number, bars: number[], plates: number[], fallbackBar: number): PlateLoad {
  return (
    bars
      .map((bar) => computePlateLoad(target, bar, plates))
      .sort(
        (a, b) =>
          Number(b.exact) - Number(a.exact) ||
          a.remainder - b.remainder ||
          b.achievedWeight - a.achievedWeight,
      )[0] ?? computePlateLoad(target, fallbackBar, plates)
  );
}
...
function BarbellSideDiagram({ load, unit }: { load: PlateLoad; unit: WeightUnit }) {
  ...
  const plates = load.perSide.flatMap((group) =>
    Array.from({ length: group.count }, (_, index) => ({ weight: group.plate, key: `${group.plate}-${index}` })),
  );
  ...
  return (
    <div className="relative min-w-0 overflow-hidden" data-testid="barbell-side-diagram">
      <div className="absolute inset-x-1 top-5 h-2 rounded-full bg-zinc-500" />
      <div className="grid min-w-0 grid-cols-[minmax(2.25rem,1.45fr)_max-content_minmax(1rem,0.85fr)]">
        <span aria-hidden />
        <div className="relative z-10 min-w-0" aria-label={t('platesPerSide')}>
          ...
          <div className="truncate text-center text-[0.625rem] text-muted-foreground">
            {t('barWeight', { weight: formatPickerNumber(load.barWeight, locale), unit: unit.toLowerCase() })}
          </div>
          {!load.exact && (
            <p className="truncate text-center text-[0.625rem] text-amber-600">
              {t('plateRemainder', { weight: formatPickerNumber(load.achievedWeight, locale), unit: unit.toLowerCase() })}
            </p>
          )}
        </div>
        <span aria-hidden />
