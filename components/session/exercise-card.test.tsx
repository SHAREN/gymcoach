...
describe('ExerciseCard', () => {
  it('keeps the title on one scrollable line and removes repeated session details', () => {
    ...
    render(<ExerciseCard programExercise={programExercise} onOpenMenu={onOpenMenu} />);
    ...
    fireEvent.click(screen.getByRole('button', { name: /exercise actions/i }));
  ...
  it('does not render the notes row for imported prescription metadata', () => {
    render(
      <ExerciseCard
        programExercise={{
          ...programExercise,
          notes: 'Alpha prescription: 5 sets; 10 reps\nAlpha metadata: Superset 1',
        }}
      />,
    );

    expect(
      screen.queryByRole('button', { name: /Notes \/ mind-muscle cue/i }),
