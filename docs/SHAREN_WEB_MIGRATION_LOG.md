...
- Exact next incomplete item after this commit: continue M15 semantic parity with selectable set-table metric views from `PROJECT_GOALS.md` — `1RM + volume` or `10RM + volume`, never 1RM and 10RM together — while confirming current-upstream equivalents before porting anything else.
