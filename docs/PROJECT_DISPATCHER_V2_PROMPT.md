...
A complete snapshot must preserve a fresh raw unfiltered
`codex_app.list_threads` envelope with `limit: 50`, `query: null`, no unavailable
hosts, and fewer than 50 returned threads. Store any temporary snapshot outside
the repository, include no raw messages, and delete it through the caller's
normal temporary-file lifecycle. The status script only reads the supplied
file.
...
2. Run the live PowerShell command and parse its stdout as JSON. Confirm that an
   incomplete thread source sets `creationRecommendationsSuppressed=true`.
