...
- DevEco project target: HarmonyOS 6.1.1, API 24, generated by the official `[Lite] Empty Ability` wizard. Huawei's Wear Engine page still documents Lite compile SDK 6 or 10, so this version mismatch remains recorded for physical-device validation.
...
- Official production sensor APIs or health permission calls. Only the verified Wear Engine messaging SDK is integrated so far.
- A verified physical-device transport. Production Wear Engine and file storage adapters compile, but signed-HAP testing still requires the owner's Huawei credentials and certificate profile.
...
huawei-watch-app/
  entry/src/main/js/MainAbility/
    ...
    pages/index/
      ...
      index.hml
      index.css
    pages/wearengine.js     # official SDK 5.0.2.306
  preview-harness/          # separate system-API-free DevEco project
  src/lite/
    production-app.js
    preview-app.js
    preview-page.js
  ...
  src/platform/
    lite-file-store.js
    watch-peer-config.js
    wear-engine-transport.js
  src/core/
    companion.js
    canonical-json.js
    contracts.js
...
The `entry/src/main/js/MainAbility` tree follows Huawei's documented Lite Wearable JavaScript, HML, and CSS application model. `npm run bundle` converts the canonical modules in `src` into a self-contained Lite runtime bundle before DevEco compilation. `entry/package.json` deliberately marks that generated tree as CommonJS so the official Lite webpack pipeline resolves its loaders instead of leaving runtime `require(...)` calls.

## Verified DevEco environment

DevEco Studio `6.1.1.280` is installed at `D:\Program Files\Huawei\DevEco Studio`. Its bundled SDK is at `D:\Program Files\Huawei\DevEco Studio\sdk`. The official Lite Wearable Wear Engine SDK `5.0.2.306` is vendored at `entry/src/main/js/MainAbility/pages/wearengine.js`; the downloaded archive SHA-256 is `64130A347647F00D734662523B5A26DACA015E87ECE7D5D916F19753759A1B56`.

The production project and the separate `preview-harness` both pass official Hvigor `assembleHap`. The Previewer harness also runs interactively on the round Lite Wearable profile and supports screen navigation, set start/completion, rest controls, summaries, diagnostics, and Russian or English labels without importing `@system.*` APIs.

Open the projects from a physical path without spaces. The verified worktree path is `D:\DevEcoProjects\GymCoachWatchIDE`. DevEco fails project sync for the repository's original path containing `projects codex`, and opening a project at the root of a substituted drive also fails because DevEco derives an invalid module name from `W:\`.

HAP signing and physical Watch GT 4 installation remain owner-controlled blockers. The unsigned build is not presented as installable on real hardware.

## Runtime boundaries

The production app bundle injects the official Wear Engine transport and Lite file-backed storage. The Previewer harness is intentionally separate and injects no Huawei device APIs; it provides deterministic visual state only and is not evidence of Bluetooth or Wear Engine connectivity.

The debug transport is isolated under `src/debug` and remains covered by Node tests. Production code uses the official Wear Engine adapter and official Lite storage implementation.
...
The build tooling uses pinned `esbuild` and Babel development dependencies to create ES5-compatible, self-contained Lite bundles. Runtime code has no npm dependency.

## Implemented Stage 6 flow

With the official transport adapter, the page flow is:
